...
data class GameSelection(val red: Int = 0, val green: Int = 0, val blue: Int = 0)

data class Game(val gameNumber: Int, val selections: List<GameSelection>)

fun parseLine(source: String): GameSelection {
    val splitColon = source.split(':').map { it.trim() }
    val gameNumber = parseGameNumber(splitColon[0])
    TODO()
}

fun parseGameNumber(source: String): Int {
    return Integer.parseInt(source.substring(5));
}